...
{
#General Packages
  dnf install -y systemd-libs | tee -a $logFile
  dnf install -y util-linux | tee -a $logFile
  dnf install -y libuuid | tee -a $logFile
  dnf install -y libselinux | tee -a $logFile
  dnf install -y pcre2 | tee -a $logFile
  dnf install -y graphviz | tee -a $logFile
  dnf install -y redhat-lsb-core | tee -a $logFile
  dnf install -y openssl | tee -a $logFile
  dnf install -y libXScrnSaver | tee -a $logFile
  dnf install -y xorg-x11-utils | tee -a $logFile
# Vitis Tools prerequisite libraries
  dnf install -y gcc | tee -a $logFile
  dnf install -y gcc-c++ | tee -a $logFile
  dnf install -y git | tee -a $logFile
  dnf install -y compat-openssl10 | tee -a $logFile
# Kernel headers and development packages
  dnf install -y kernel-headers-$(uname -r) | tee -a $logFile
  dnf install -y kernel-devel-$(uname -r) | tee -a $logFile
# X virtual framebuffer (for GUI apps in headless environments)
  dnf install -y xorg-x11-server-Xvfb | tee -a $logFile
}
...
### Vitis Tools prerequisite libraries
   ...
   apt-get install -y openssl | tee -a $logFile
...
### Vitis Tools prerequisite libraries
    ...
    #yum install -y compat-openssl10  | tee -a $logFile
